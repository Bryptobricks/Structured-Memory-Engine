 ...
 * @returns {{ since: string|null, until: string|null, recencyBoost: number|null, dateTerms: string[], strippedQuery: string, forwardLooking: boolean }}
 */
function resolveTemporalQuery(query, now = new Date()) {
  ...
  if (!forwardLooking && /\b(plan|plans|planned|planning|goal|goals|schedule|scheduled|upcoming|deadline|deadlines|due|milestones?|todo|to-do)\b/i.test(query)) {
    forwardLooking = true;
  }

  strippedQuery = strippedQuery.replace(/\s+/g, ' ').replace(/\s+([?!.,;:])/g, '$1').trim();

  return { since, until, recencyBoost, dateTerms, strippedQuery, forwardLooking };
}
