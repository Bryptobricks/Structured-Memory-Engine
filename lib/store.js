...
function openDb(workspace) {
  ...
  try { db.exec('ALTER TABLE chunks ADD COLUMN priority TEXT DEFAULT \'medium\''); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN referenced_date TEXT'); } catch (_) { /* migration: column may already exist */ }
  try { db.exec('ALTER TABLE chunks ADD COLUMN relative_offset REAL'); } catch (_) { /* migration: column may already exist */ }
...
function insertChunks(db, filePath, mtimeMs, chunks, createdAt, dedupConfig, sourceType = 'indexed') {
  ...
  const insert = db.prepare(`
    INSERT INTO chunks (file_path, heading, content, line_start, line_end, entities, chunk_type, confidence, created_at, indexed_at, file_weight, content_updated_at, source_type, domain, priority, referenced_date, relative_offset)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const tx = db.transaction(() => {
    ...
    for (const c of chunksToInsert) {
      ...
      insert.run(filePath, c.heading || null, c.content, c.lineStart, c.lineEnd, JSON.stringify(c.entities), chunkType, confidence, createdAt || now, now, weight, contentUpdatedAt, sourceType, c.domain || 'general', c.priority || 'medium', c.referencedDate || null, c.relativeOffset != null ? c.relativeOffset : null);
    }
