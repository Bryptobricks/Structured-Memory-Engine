...
function insertChunks(db, filePath, mtimeMs, chunks, createdAt, dedupConfig) {
  ...
  const insert = db.prepare(`
    INSERT INTO chunks (file_path, heading, content, line_start, line_end, entities, chunk_type, confidence, created_at, indexed_at, file_weight, content_updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const tx = db.transaction(() => {
    ...
    for (const c of chunksToInsert) {
      ...
      insert.run(filePath, c.heading || null, c.content, c.lineStart, c.lineEnd, JSON.stringify(c.entities), chunkType, confidence, createdAt || now, now, weight, contentUpdatedAt);
    }
