...
if (!command || command === 'help') {
  ...
  node lib/index.js query "search terms" [--limit N] [--since 7d|2w|3m|1y|2026-01-01] [--context N]
...
if (command === 'index') {
  ...
  const results = recall(db, query, { limit: args.limit, since: args.since, context: args.context || 0, workspace });
  if (results.length === 0) {
    console.log('No results found.');
  } else {
    for (const r of results) {
      console.log(`\n--- ${r.filePath}:${r.lineStart}-${r.lineEnd} ${r.heading ? '(' + r.heading + ')' : ''} [fts: ${r.ftsScore?.toFixed(4)} final: ${r.finalScore?.toFixed(4)} weight: ${r.fileWeight}]`);
