...
if (!command || command === 'help') {
  ...
  node lib/index.js query "search terms" [--limit N] [--since 7d|2w|3m|1y|2026-01-01] [--context N] [--type fact|confirmed|inferred|outdated|decision|preference|opinion] [--min-confidence 0.5]
...
if (command === 'index') {
  ...
  const results = recall(db, query, { limit: args.limit, since: args.since, context: args.context || 0, workspace, chunkType: args.type || null, minConfidence: args.minConfidence != null ? args.minConfidence : null });
  if (results.length === 0) {
