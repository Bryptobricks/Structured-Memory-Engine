...
function recall(db, query, { limit = 10, since = null, context = 0, workspace = null, chunkType = null, minConfidence = null } = {}) {
  const aliases = loadAliases(workspace);
  const sinceDate = parseSince(since);
  const sanitized = sanitizeFtsQuery(query);
  if (!sanitized) return [];
  const searchOpts = { limit, sinceDate, chunkType, minConfidence };
  try {
