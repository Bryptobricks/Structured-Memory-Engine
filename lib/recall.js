...
function recall(db, query, { limit = 10, since = null, context = 0, workspace = null, chunkType = null, minConfidence = null, includeStale = false, excludeFromRecall: excludePatterns = null, queryEmbedding = null } = {}) {
  ...
  const fetchLimit = queryEmbedding ? limit * 3 : limit;
  const searchOpts = { limit: fetchLimit, sinceDate, chunkType, minConfidence, includeStale };
  ...
  try {
    ...
    if (queryEmbedding && queryEmbedding.length > 0) {
      ...
      for (const row of allEmbedded) {
        if (rescueCount >= RESCUE_MAX) break;
        ...
        // Partial FTS credit for rescued chunks
        row.rank = 0;
    ...
    const results = rankResults(rows, activeProfile);
