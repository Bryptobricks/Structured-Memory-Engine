...
function buildOrQuery(query, aliases) {
  if (!query || !query.trim()) return null;
  const rawTerms = query.split(/\s+/).filter(Boolean).map(t => t.replace(/"/g, ''));
  // Expand with aliases
...
function rankResults(rows) {
  ...
  return rows.map(r => {
    ...
    const confidence = r.confidence != null ? r.confidence : 1.0;
    const finalScore = ftsScore * (1 + recencyBoost) * fileWeight * confidence;
    return {
      content: r.content,
      heading: r.heading,
...
function recall(db, query, { limit = 10, since = null, context = 0, workspace = null, chunkType = null, minConfidence = null, includeStale = false } = {}) {
...
module.exports = { recall, parseSince, sanitizeFtsQuery, buildOrQuery, rankResults, loadAliases };
