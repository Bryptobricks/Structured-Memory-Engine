...
const { runReflectCycle, getLastReflectTime, listContradictions, resolveContradiction } = require('./reflect');
const { remember } = require('./remember');
const { loadConfig, resolveIncludes } = require('./config');
const { getRelevantContext } = require('./context');
const { buildEntityIndex, getEntity, listEntities, getRelatedEntities, generateEntityPage, generateEntityPages, entityFilename } = require('./entities');
...
async function handleIndex(db, workspace, args, config) {
  ...
  if (config && config.reflect && config.reflect.autoReflectOnIndex) {
    const reflectResult = runReflectCycle(db, { config, workspace });
...
async function handleReflect(db, args, config, workspace) {
  const result = runReflectCycle(db, { dryRun: args.dryRun || false, config: config || null, mode: args.mode || null, workspace });
...
async function main() {
  ...
  // Auto-reflect helper: run reflect if enough time has elapsed
  const reflectIntervalMs = config.reflect.intervalHours * 60 * 60 * 1000;
  const CHECK_INTERVAL_MS = 60 * 60 * 1000; // check every hour

  function maybeReflect() {
    try {
      const lastReflect = getLastReflectTime(workspace);
      if (Date.now() - lastReflect > reflectIntervalMs) {
        const reflectResult = runReflectCycle(db, { config, workspace });
        ...
        if (reflectResult.entityPages && reflectResult.entityPages.generated > 0) {
          log(`Entity pages: generated ${reflectResult.entityPages.generated}`);
        }
      }
    } catch (err) {
  ...
  // Run on startup + schedule recurring checks (gated on autoReflect config)
  let reflectTimer = null;
  if (config.reflect.autoReflect !== false) {
    maybeReflect();
    reflectTimer = setInterval(maybeReflect, CHECK_INTERVAL_MS);
    reflectTimer.unref();
  } else {
    log('Auto-reflect: disabled by config');
  }

  // Graceful shutdown — close DB handle, checkpoint WAL, clear timers
  function shutdown() {
    log('Shutting down...');
    if (reflectTimer) { clearInterval(reflectTimer); reflectTimer = null; }
  ...
  server.tool(
    ...
    `Query the entity graph from ${ownerLabel} memory. Look up a specific person, project, or concept to see related entities and which memories mention them. Without a name, lists all known entities.`,
    {
      ...
      generate: z.boolean().optional().describe('Generate markdown summary pages in .memory/entities/. With name: one page. Without: all entities with 2+ mentions.'),
    },
    async (args) => {
      try {
        if (args.generate) {
          const outputDir = path.join(workspace, '.memory', 'entities');
          if (args.name) {
            const page = generateEntityPage(db, args.name);
            if (!page) return { content: [{ type: 'text', text: `Entity "${args.name}" not found.` }] };
            fs.mkdirSync(outputDir, { recursive: true });
            fs.writeFileSync(path.join(outputDir, entityFilename(args.name)), page);
            return { content: [{ type: 'text', text: page }] };
          }
          const epConfig = (config && config.entityPages) || {};
          const result = generateEntityPages(db, outputDir, { minMentions: epConfig.minMentions || 2, limit: epConfig.limit || 50 });
          let text = `Generated ${result.generated} entity pages in .memory/entities/\n`;
          for (const p of result.pages) text += `  ${p.entity} → ${p.file}\n`;
          return { content: [{ type: 'text', text: text.trim() }] };
        }
        if (args.name) {
          if (args.related) {
