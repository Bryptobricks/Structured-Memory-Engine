...
async function handleReflect(db, args, config, workspace) {
  ...
  if (result.restructure && result.restructure.total > 0) {
    text += `  Restructured: ${result.restructure.total} (superseded=${result.restructure.superseded.length} duplicates=${result.restructure.duplicates.length})\n`;
  }
