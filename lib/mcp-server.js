...
async function main() {
  ...
  server.tool(
    ...
    {
      message: z.string().describe('The user message to find context for'),
      maxTokens: z.number().optional().describe('Token budget for context (default 1500)'),
      conversationContext: z.array(z.string()).optional().describe('Recent user messages for multi-turn awareness (last 2-3 turns)'),
    },
    async (args) => {
      try {
  ...
  server.tool(
    ...
    {
      name: z.string().optional().describe('Entity name to look up (e.g. "Jason", "MovePosition"). Omit to list all entities.'),
