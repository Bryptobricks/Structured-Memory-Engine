...
function indexSingleFile(db, workspace, filePath) {
...
function handleRemember(db, workspace, args) {
  ...
  try {
    indexSingleFile(db, workspace, result.filePath);
  } catch (err) {
...
function handleIndex(db, workspace, args, config) {
  ...
  const result = indexWorkspace(db, workspace, { force: args.force || false, include });
  let text = `Indexed: ${result.indexed} | Skipped: ${result.skipped} | Total: ${result.total}`;
...
async function main() {
  ...
  try {
    ...
    log(`Auto-index: indexed=${result.indexed} skipped=${result.skipped} total=${result.total}`);
  } catch (err) {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        return handleRemember(db, workspace, args);
      } catch (err) {
...
module.exports = { handleQuery, handleRemember, handleIndex, handleReflect, handleStatus, indexSingleFile };
