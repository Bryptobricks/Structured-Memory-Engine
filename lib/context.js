...
function getRelevantContext(db, message, opts = {}) {
  ...
  const maxChunks = (intent && intent.maxChunks) || maxChunksOpt;
  const minCilScore = (intent && intent.minCilScore != null) ? intent.minCilScore : minCilScoreOpt;
  ...
  if (queryEmbedding && queryEmbedding.length > 0) {
    ...
    if (rescueCandidates.length > 0) {
      ...
      for (const rc of rescueChunks) {
        ...
        rc._normalizedFts = rc._semanticSim * 0.6; // cosine-weighted partial credit for rescued chunks
        rc.rank = 0;
        results.push(rc);
        rescuedCount++;
      }
...
module.exports = { getRelevantContext, extractQueryTerms, cilScore, budgetChunks, formatContext, invalidateEntityCache, detectQueryIntent };
