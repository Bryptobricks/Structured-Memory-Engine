...
const PRIORITY_DECAY_MULTIPLIER = {
  high: 0.5,
  medium: 1.0,
  low: 1.5,
};
...
function decayConfidence(db, { dryRun = false, config = null } = {}) {
  ...
  const rows = db.prepare('SELECT id, heading, chunk_type, confidence, last_accessed, created_at, content_updated_at, priority FROM chunks WHERE confidence > 0 AND chunk_type != ?').all('confirmed');
  const details = [];
  const updates = [];
  ...
  for (const row of rows) {
    ...
    const priorityDecay = PRIORITY_DECAY_MULTIPLIER[row.priority] || 1.0;
    const decayAmount = (daysSince / halfLife) * rate * rateMultiplier * priorityDecay * 0.5;
...
// --- v9: Restructuring — supersession detection ---
...
function parseEntities(entitiesStr) {
  if (!entitiesStr) return [];
  try { return JSON.parse(entitiesStr); } catch (_) { return []; }
}

function getPrimaryEntity(entities) {
  if (!entities || entities.length === 0) return null;
  // First non-acronym entity, or first entity if all are acronyms
  return entities.find(e => !/^[A-Z$@]/.test(e)) || entities[0] || null;
}

/**
 * Compute similarity between two chunks based on entity overlap, term overlap, and same-file bonus.
 */
function computeChunkSimilarity(a, b) {
  const entitiesA = new Set((parseEntities(a.entities) || []).map(e => e.toLowerCase()));
  const entitiesB = new Set((parseEntities(b.entities) || []).map(e => e.toLowerCase()));
  const entityOverlap = jaccardSimilarity(entitiesA, entitiesB);

  const termsA = new Set(extractTerms(a.content));
  const termsB = new Set(extractTerms(b.content));
  const termOverlap = jaccardSimilarity(termsA, termsB);

  const sameFile = a.file_path === b.file_path ? 0.1 : 0;

  return entityOverlap * 0.5 + termOverlap * 0.4 + sameFile;
}

/**
 * Detect superseded chunks — newer facts/preferences that replace older ones about the same entity.
 * Returns { superseded: [...], duplicates: [...] }
 */
function detectSuperseded(db, { dryRun = false, recencyWindowDays = 90, similarityThreshold = 0.5 } = {}) {
  const now = Date.now();
  const SUPERSEDABLE_TYPES = new Set(['fact', 'preference']);
  const rows = db.prepare(
    'SELECT id, content, entities, chunk_type, confidence, created_at, file_path FROM chunks WHERE stale = 0 AND chunk_type IN (?, ?) ORDER BY created_at ASC'
  ).all('fact', 'preference');
  ...
  for (const key of Object.keys(groups)) {
    ...
    for (let i = 0; i < group.length; i++) {
      for (let j = i + 1; j < group.length; j++) {
        ...
        // Recency window: only compare if newer is within N days of older
        const olderMs = older.created_at ? new Date(older.created_at).getTime() : 0;
        const newerMs = newer.created_at ? new Date(newer.created_at).getTime() : 0;
        if ((newerMs - olderMs) > recencyWindowDays * 86400000) continue;

        const sim = computeChunkSimilarity(older, newer);
        if (sim < similarityThreshold) continue;

        // High similarity (>0.7) = duplicate — mark older as outdated
        if (sim > 0.7) {
          duplicates.push({ oldId: older.id, newId: newer.id, similarity: sim, reason: `duplicate (sim=${sim.toFixed(3)})` });
          toUpdate.push(older.id);
          continue;
        }

        // Negation/change in newer = supersession
        if (NEGATION_PATTERN.test(newer.content)) {
          superseded.push({ oldId: older.id, newId: newer.id, similarity: sim, reason: `superseded by negation (sim=${sim.toFixed(3)})` });
          toUpdate.push(older.id);
          continue;
        }

        // Newer has higher confidence = supersession
        if (newer.confidence > older.confidence) {
          superseded.push({ oldId: older.id, newId: newer.id, similarity: sim, reason: `superseded by higher confidence (sim=${sim.toFixed(3)})` });
          toUpdate.push(older.id);
        }
      }
    }
  ...
  if (!dryRun && toUpdate.length > 0) {
    ...
    const tx = db.transaction(() => {
      for (const id of toUpdate) stmt.run('outdated', id);
    });
    tx();
  }

  return { superseded, duplicates, total: superseded.length + duplicates.length };
}
...
function runReflectCycle(db, { dryRun = false, config = null, mode = null } = {}) {
  ...
  // v9: Restructure — detect and mark superseded chunks
  let restructure = { superseded: [], duplicates: [], total: 0 };
  try {
    restructure = detectSuperseded(db, { dryRun: effectiveDryRun });
  } catch (err) { console.debug('[sme:reflect] restructure failed:', err.message); }
  ...
  return { decay, reinforce, stale, contradictions, restructure, prune, entityIndex, valueAssessment: valueAssessmentResult, retroDedup, mode: effectiveMode };
}
...
module.exports = { decayConfidence, reinforceConfidence, markStale, detectContradictions, detectSuperseded, computeChunkSimilarity, pruneStale, restoreChunk, resolveContradiction, runReflectCycle, listContradictions, extractTerms, extractDateFromPath, negationNearSharedTerm, getLastReflectTime, setLastReflectTime, valueAssessment, retroDedupScan, REVIEW_VERSION, PRIORITY_DECAY_MULTIPLIER };
