...
function runReflectCycle(db, { dryRun = false, config = null, mode = null, workspace = null } = {}) {
  ...
  // Auto-generate entity pages if enabled
  let entityPages = { generated: 0, total: 0, pages: [] };
  const epConfig = (config && config.entityPages) || {};
  if (epConfig.enabled && workspace && !effectiveDryRun) {
    try {
      const path = require('path');
      const { generateEntityPages } = require('./entities');
      const outputDir = path.join(workspace, '.memory', 'entities');
      entityPages = generateEntityPages(db, outputDir, { minMentions: epConfig.minMentions || 2, limit: epConfig.limit || 50 });
    } catch (err) { console.debug('[sme:reflect] entity page generation failed:', err.message); }
  }

  // Record reflect timestamp so auto-reflect timers know when we last ran
  if (effectiveMode === 'apply' && workspace) {
    try { setLastReflectTime(workspace); } catch (_) { /* non-fatal */ }
  }

  return { decay, reinforce, stale, contradictions, prune, entityIndex, entityPages, valueAssessment: valueAssessmentResult, retroDedup, mode: effectiveMode };
}
