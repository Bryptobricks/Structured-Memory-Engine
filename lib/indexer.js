...
function indexWorkspace(db, workspace, { force = false, include = [] } = {}) {
  ...
  for (const filePath of files) {
    try {
      ...
      if (facts.length > 0) {
        // Build a line→fact lookup for fast matching
        const factByLine = new Map();
        for (const f of facts) factByLine.set(f.lineStart, f);

        for (const chunk of chunks) {
          // Find the best (highest confidence) fact that falls within this chunk's line range
          let bestFact = null;
        ...
        // Also add individual fact chunks for tagged single-line facts
        // that may be buried in larger chunks — gives them standalone searchability
        for (const f of facts) {
          // Only add standalone fact chunks for explicitly tagged items (not heading-derived)
          if (f.content.length > 0 && f.content.length < 500) {
            chunks.push({
              heading: null,
              content: f.content,
