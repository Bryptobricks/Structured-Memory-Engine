...
function indexWorkspace(db, workspace, { force = false, include = [], fileTypeDefaults = {}, excludePatterns = [] } = {}) {
  ...
  for (const filePath of files) {
    ...
    try {
      ...
      // v7.1: Auto-classify remaining raw chunks by explicit tags in content
      for (const chunk of chunks) {
        if (!chunk.chunkType || chunk.chunkType === 'raw') {
          const classified = classifyChunk(chunk.content);
          if (classified) chunk.chunkType = classified;
        }
      }
 ...
 */
function indexSingleFile(db, workspace, filePath, fileTypeDefaults, excludePatterns) {
  const stat = fs.statSync(filePath);
  const mtimeMs = Math.floor(stat.mtimeMs);
  const relPath = path.relative(workspace, filePath);
...
module.exports = { indexWorkspace, indexSingleFile, chunkMarkdown, extractEntities, discoverFiles, classifyChunk };
